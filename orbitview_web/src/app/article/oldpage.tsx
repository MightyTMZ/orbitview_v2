...
const page = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://127.0.0.1:8000/blog/articles/blog/"
        );
        if (!response.ok) {
  ...
  /*const response = await fetch("http://127.0.0.1:8000/blog/articles/blog/", {
    cache: "no-store",
  });
  ...
  if (data) {
    return (
      <div>
        <Article article={data["results"][0]} />
      </div>
    );
  }
  return <div>HTTP 404. Not found</div>;
};
